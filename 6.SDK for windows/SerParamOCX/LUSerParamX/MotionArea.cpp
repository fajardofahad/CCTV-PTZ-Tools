...
void CMotionArea::OnPaint()
{
	...
	else
	{
		...
		for(i=0;i<4;i++)
		{
			if(i != m_curmaskarea)
			{
				rect.left = pvmask[i]->m_x;
				rect.right = pvmask[i]->m_x + pvmask[i]->m_width;
				rect.top = pvmask[i]->m_y;
				rect.bottom = pvmask[i]->m_y + pvmask[i]->m_height;
				pDc->FrameRect(&rect,&BrushBlue);
			}
		}
	}
}
...
void CMotionArea::OnMouseMove(UINT nFlags, CPoint point) 
{
	...
	else 
	{
		if(m_bVideoMaskBtnDown && m_bLinkOk && nFlags == MK_LBUTTON)
		{
			...
			if(m_vmask_w > 0)
			{
				pvmask[m_curmaskarea]->m_x     = m_vmask_x;
				pvmask[m_curmaskarea]->m_width = m_vmask_w;
			}
			else
			{
				pvmask[m_curmaskarea]->m_x		= m_vmask_x + m_vmask_w;
				pvmask[m_curmaskarea]->m_width = -m_vmask_w;
			}
			if(m_vmask_h > 0)
			{
				pvmask[m_curmaskarea]->m_y      = m_vmask_y;
				pvmask[m_curmaskarea]->m_height = m_vmask_h;
			}
			else
			{
				pvmask[m_curmaskarea]->m_y      = m_vmask_y + m_vmask_h;
				pvmask[m_curmaskarea]->m_height = - m_vmask_h;
			}
		}
		Invalidate();
...
void CMotionArea::Reinit(BOOL bchange)
{
	if(!m_bIsArea)
	{
		if(bchange)
		{
			...
			for(i=0;i<4;i++)
			{
				pvmask[i]->m_x = (short)(((float)pvmask[i]->m_x) * w_off);
				pvmask[i]->m_y = (short)(((float)pvmask[i]->m_y) * h_off);
				pvmask[i]->m_width = (short)(((float)pvmask[i]->m_width) * w_off);
				pvmask[i]->m_height = (short)(((float)pvmask[i]->m_height) * h_off);
			}
		}
		m_vmask_x = pvmask[m_curmaskarea]->m_x;
		m_vmask_y = pvmask[m_curmaskarea]->m_y;
		m_vmask_w = pvmask[m_curmaskarea]->m_width;
		m_vmask_h = pvmask[m_curmaskarea]->m_height;
	}
}
